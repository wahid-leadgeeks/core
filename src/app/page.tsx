...
export default function DashboardPage() {
  ...
  const cards = [
    ...
    },
    {
      ...
      count: stats.devices,
      subtext: `${stats.assignedDevices} Assigned • ${stats.reserveDevices} Reserve • ${stats.availableDevices} Available`,
      icon: Laptop,
      href: '/assets',
    },
    {
    ...
    <AppShell>
      <div className="space-y-6">
          ...
          {cards.map((card) => {
            ...
            return (
              <Link
                key={card.title}
                href={card.href}
                className="p-5 rounded-xl border border-slate-800 bg-[#0a0f1d] hover:border-slate-700 hover:bg-slate-900/60 transition-all group flex flex-col justify-between space-y-4 shadow-sm"
              >
                <div className="flex items-start justify-between">
                  <div className="p-2.5 rounded-lg bg-slate-800 border border-slate-700/60 text-slate-300 group-hover:text-emerald-400 transition-colors">
                    <Icon size={18} />
                  </div>
                  <ArrowRight
                    size={16}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="p-5 rounded-xl border border-slate-800 bg-[#0a0f1d] flex flex-col justify-between space-y-4 shadow-sm">
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
                ...
                <span className="text-[11px] font-mono text-amber-400 px-2 py-0.5 rounded bg-amber-950/60 border border-amber-800/80">
                  Action Required
                </span>
              </div>

              <ul className="space-y-3 text-xs font-sans">
                <li className="p-2.5 rounded-lg bg-slate-900/60 border border-slate-800/80 space-y-1">
                  <div className="flex items-center justify-between font-mono">
                    <span className="font-semibold text-slate-200">Google Workspace Sync</span>
                    <span className="text-[10px] text-amber-400">15 Groups</span>
                  </div>
                  ...
                  <Link
                    href="/groups/matrix"
                    className="inline-flex items-center gap-1 text-[11px] font-mono text-emerald-400 hover:underline pt-0.5"
                  >
                ...
                <li className="p-2.5 rounded-lg bg-slate-900/60 border border-slate-800/80 space-y-1">
                  <div className="flex items-center justify-between font-mono">
                    <span className="font-semibold text-slate-200">Reserve Hardware Pool</span>
                    <span className="text-[10px] text-emerald-400">2 Devices</span>
                  </div>
                  <p className="text-slate-400 text-[11px] leading-tight">
          ...
          <div className="p-5 rounded-xl border border-slate-800 bg-[#0a0f1d] flex flex-col justify-between space-y-4 shadow-sm">
            <div className="space-y-3">
              ...
              {auditForbidden ? (
                <div className="p-4 rounded-lg bg-slate-900/40 border border-slate-800 text-center space-y-1.5 py-6">
                  <Lock size={20} className="mx-auto text-slate-400" />
                  <p className="text-xs text-slate-300 font-mono">Access Gated (ADR-005)</p>
                  <p className="text-[11px] text-slate-400">
                    Audit event stream is restricted exclusively to Super Admin and Auditor roles.
                  </p>
                </div>
              ) : recentAudit.length === 0 ? (
                <div className="p-4 rounded-lg bg-slate-900/40 border border-slate-800 text-center space-y-1 py-6">
                  <CheckCircle2 size={20} className="mx-auto text-slate-400" />
                  <p className="text-xs text-slate-400 font-mono">Audit stream logging active</p>
                  <p className="text-[11px] text-slate-400">
            ...
            <div className="pt-2 border-t border-slate-800/60 text-[11px] font-mono text-slate-400 flex items-center justify-between">
              ...
              <span className="text-slate-300">Append-Only</span>
            </div>
          </div>
          ...
          <div className="p-5 rounded-xl border border-slate-800 bg-[#0a0f1d] flex flex-col justify-between space-y-4 shadow-sm">
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-xs">
                <Link
                  href="/groups/matrix"
                  className="flex items-center justify-between p-2.5 rounded-lg bg-slate-900/60 border border-slate-800/80 hover:border-slate-700 hover:bg-slate-900 transition-all group"
                >
            ...
            <div className="pt-2 border-t border-slate-800/60 text-[11px] font-mono text-slate-400 flex items-center justify-between">
              <span>RBAC Mode</span>
              <span className="text-emerald-400">Edge Guard Active</span>
            </div>
          </div>
        </div>
      </div>
