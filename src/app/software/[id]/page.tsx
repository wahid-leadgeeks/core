      ...
      <div className="space-y-5">
        ...
        <div className="p-6 rounded-xl border border-slate-800 bg-[#0a0f1d] shadow-lg space-y-4">
          ...
          {application.description && (
            <p className="pt-2 border-t border-slate-800/60 text-xs text-slate-300 font-sans leading-relaxed">
              {application.description}
            </p>
          )}
        </div>
        ...
        {/* Tab Navigation */}
        <div role="tablist" aria-label="Software details tabs" className="flex items-center gap-1 border-b border-slate-800 pb-px">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              role="tab"
        ...
        {activeTab === 'overview' && (
          <div role="tabpanel" id="panel-overview" aria-labelledby="tab-overview" className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="p-5 rounded-xl border border-slate-800 bg-[#0a0f1d] space-y-4">
              ...
              <dl className="space-y-3 text-xs font-mono">
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  ...
                  <dd className="text-slate-200 font-medium">{application.name}</dd>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  <dt className="text-slate-400">Operational Category</dt>
                  <dd className="text-slate-300 capitalize">{application.category || 'General'}</dd>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  <dt className="text-slate-400">Lifecycle Status</dt>
                  <dd className="text-emerald-400 capitalize">{application.status}</dd>
                </div>
                <div className="flex justify-between py-1.5">
                  <dt className="text-slate-400">Unique Resource ID</dt>
                  <dd className="text-slate-400 text-[11px] select-all">{application.id}</dd>
                </div>
              </dl>
            </div>
            ...
            <div className="p-5 rounded-xl border border-slate-800 bg-[#0a0f1d] space-y-4">
              ...
              <dl className="space-y-3 text-xs font-mono">
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  <dt className="text-slate-400">Application Website</dt>
                  <dd className="text-slate-300 truncate max-w-[240px]">
                    {application.websiteUrl || 'Not specified'}
                  </dd>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  <dt className="text-slate-400">Authorization Model</dt>
                  <dd className="text-slate-300">Department / Role-gated</dd>
                </div>
                <div className="flex justify-between py-1.5">
                  <dt className="text-slate-400">SSO / OAuth Integration</dt>
                  <dd className="text-emerald-400">Google Workspace OIDC</dd>
                </div>
              </dl>
            </div>
          </div>
        ...
        {activeTab === 'subscription' && (
          <div role="tabpanel" id="panel-subscription" aria-labelledby="tab-subscription" className="p-6 rounded-xl border border-slate-800 bg-[#0a0f1d] space-y-5">
            <h3 className="text-sm font-mono font-semibold text-slate-200 uppercase tracking-wider">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs font-mono">
              <div className="p-4 rounded-xl border border-slate-800 bg-slate-900/50 space-y-1.5">
                <div className="text-slate-400 text-[11px] uppercase">Plan Tier</div>
                <div className="text-base font-semibold text-slate-100 capitalize">
              ...
              <div className="p-4 rounded-xl border border-slate-800 bg-slate-900/50 space-y-1.5">
                <div className="text-slate-400 text-[11px] uppercase">Cost Allocation</div>
              ...
              <div className="p-4 rounded-xl border border-slate-800 bg-slate-900/50 space-y-1.5">
                <div className="text-slate-400 text-[11px] uppercase">Compliance State</div>
                <div className="text-base font-semibold text-emerald-400">Active Approved</div>
                <div className="text-[11px] text-slate-400">Authorized in company software inventory</div>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'department' && (
          <div role="tabpanel" id="panel-department" aria-labelledby="tab-department" className="p-6 rounded-xl border border-slate-800 bg-[#0a0f1d] space-y-5">
            <h3 className="text-sm font-mono font-semibold text-slate-200 uppercase tracking-wider">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs font-mono">
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-slate-800/60">
                  ...
                  <span className="text-slate-200 font-medium">{application.departmentName}</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-slate-400">Department Code</span>
                  <span className="text-slate-300 font-bold">{application.departmentCode}</span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-slate-800/60">
                  ...
                  <span className="text-slate-300">Software Admin (GRW)</span>
                </div>
                <div className="flex justify-between py-2">
