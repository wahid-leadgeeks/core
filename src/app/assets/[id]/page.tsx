  ...
  return (
    <AppShell>
      <div className="space-y-5">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    >
                      {copiedAsset ? <Check size={13} className="text-emerald-400" /> : <Copy size={13} />}
                    </button>
                    <StatusBadge status={device.status} size="sm" />
                  </div>
                  <div className="text-xs text-slate-400 font-sans mt-0.5 flex items-center gap-1.5 flex-wrap">
                    ...
                    {device.computerName && (
                      <button
                        ...
                        className="p-0.5 rounded text-slate-400 hover:text-white transition-colors inline-flex items-center"
                        title="Copy Hostname"
                        aria-label="Copy Hostname"
                      >
                        {copiedHost ? <Check size={11} className="text-emerald-400" /> : <Copy size={11} />}
                      </button>
                    )}
                  </div>
                </div>
              ...
              <div className="flex items-center gap-4 text-xs font-mono text-slate-400 pt-1 flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>Assignee: {assignment?.assigneeName || 'Unassigned'}</span>
                  {assignment?.assigneeEmail && (
                    ...
                    >
                      {copiedAssignee ? <Check size={11} className="text-emerald-400" /> : <Copy size={11} />}
                    </button>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1.5">
                  <Shield size={13} className={device.hasAntivirus ? 'text-emerald-400' : 'text-amber-400'} />
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {canUpdate && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-700 text-xs font-medium text-slate-200 hover:bg-slate-800 hover:border-slate-600 transition-colors"
                >
              ...
              {canDecommission && device.status !== 'decommissioned' && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-950/40 border border-rose-900/60 text-xs font-medium text-rose-300 hover:bg-rose-900/60 transition-colors"
                >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-950/80 border border-amber-800 text-xs font-mono text-amber-300 hover:bg-amber-900 transition-colors"
        ...
        {/* Tab Navigation */}
        <div role="tablist" aria-label="Device details tabs" className="flex items-center gap-1 border-b border-slate-800 pb-px overflow-x-auto">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              role="tab"
            ...
            <div className="p-5 rounded-xl border border-slate-800 bg-[#0a0f1d] space-y-4">
              ...
              <dl className="space-y-3 text-xs font-mono">
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  <dt className="text-slate-500">Asset Tag ID</dt>
                  <dd className="text-slate-200 font-bold">{device.assetNumber}</dd>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  <dt className="text-slate-500">Brand</dt>
                  <dd className="text-slate-300">{device.brand || 'N/A'}</dd>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  <dt className="text-slate-500">Model Name</dt>
                  <dd className="text-slate-200">{device.model}</dd>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  <dt className="text-slate-500">Computer Hostname</dt>
                  <dd className="text-slate-300">{device.computerName || 'N/A'}</dd>
                </div>
                <div className="flex justify-between py-1.5">
                  <dt className="text-slate-500">System Status</dt>
                  <dd className="text-emerald-400 capitalize">{device.status}</dd>
                </div>
              </dl>
            </div>
            ...
            <div className="p-5 rounded-xl border border-slate-800 bg-[#0a0f1d] space-y-4">
              ...
              <dl className="space-y-3 text-xs font-mono">
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  ...
                  <dd className="text-slate-300">{device.purchasedAt || 'N/A'}</dd>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  <dt className="text-slate-500">Antivirus Software</dt>
                  <dd className={device.hasAntivirus ? 'text-emerald-400' : 'text-amber-400'}>
                    {device.hasAntivirus ? 'Installed & Active' : 'Not Installed'}
                  </dd>
                </div>
                <div className="flex justify-between py-1.5 border-b border-slate-800/60">
                  <dt className="text-slate-500">Internal ID</dt>
                  <dd className="text-slate-400 text-[11px] select-all">{device.id}</dd>
                </div>
                <div className="flex justify-between py-1.5">
                  <dt className="text-slate-500">Credential Vault</dt>
                  <dd className="text-emerald-400">AES-256-GCM Encrypted</dd>
                </div>
              </dl>
            </div>
          </div>
        ...
        {activeTab === 'specs' && (
          <div role="tabpanel" id="panel-specs" aria-labelledby="tab-specs" className="p-6 rounded-xl border border-slate-800 bg-[#0a0f1d] space-y-6">
            <h3 className="text-sm font-mono font-semibold text-slate-200 uppercase tracking-wider">
        ...
        {activeTab === 'assignment' && (
          <div role="tabpanel" id="panel-assignment" aria-labelledby="tab-assignment" className="p-6 rounded-xl border border-slate-800 bg-[#0a0f1d] space-y-6">
            <h3 className="text-sm font-mono font-semibold text-slate-200 uppercase tracking-wider">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs font-mono">
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-slate-800/60">
                  <span className="text-slate-400">Primary PIC (Assignee)</span>
              ...
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-slate-800/60">
                  <span className="text-slate-400">Secondary PIC (Custodian)</span>
                  <span className="text-slate-300">
                ...
                <div className="flex justify-between py-2">
                  ...
                  <span className="text-emerald-400">Active Checked Out</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'access' && (
          <div role="tabpanel" id="panel-access" aria-labelledby="tab-access" className="p-6 rounded-xl border border-slate-800 bg-[#0a0f1d] space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => setPinModalOpen(true)}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-semibold text-xs font-mono transition-colors shadow-sm"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs font-mono">
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-slate-800/60">
                  <span className="text-slate-400">Local Login Email</span>
                  <span className="text-slate-200">{credential?.loginEmail || 'N/A'}</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-slate-400">PIN Password Status</span>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between py-2">
                  ...
                  <span className="text-emerald-400">Strict Append-Only Logging</span>
                </div>
              </div>
            </div>
          </div>
