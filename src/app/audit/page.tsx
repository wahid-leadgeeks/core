...
export default function AuditPage() {
  ...
  const getActionBadgeClasses = (action: string) => {
    if (action.includes('reveal')) return 'bg-amber-950/80 text-amber-300 border-amber-800';
    ...
    return (
      <AppShell>
        <div className="max-w-2xl mx-auto py-16 text-center space-y-4">
          <div className="inline-flex p-4 rounded-2xl bg-rose-950/40 border border-rose-900 text-rose-400">
            <Lock size={32} />
          </div>
          ...
          <p className="text-sm text-slate-400 font-sans leading-relaxed">
            ...
            <span className="text-slate-200 font-semibold">Super Admin</span> and{' '}
            <span className="text-slate-200 font-semibold">Auditor</span> roles.
          </p>
          <div className="pt-2 text-xs font-mono text-slate-400">
            Current role: <span className="text-amber-400 uppercase">{user?.role || 'Restricted'}</span>
          </div>
        </div>
      </AppShell>
    );
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-xs font-mono text-slate-300 hover:text-white transition-colors self-start md:self-auto"
          >
        ...
        {/* Filter Controls */}
        <div className="space-y-3 bg-[#0a0f1d] p-4 rounded-xl border border-slate-800">
          ...
          {/* Action Filter Pills */}
          <div className="pt-2 border-t border-slate-800/60 flex items-center gap-1.5 overflow-x-auto text-xs font-mono pb-1">
            <span className="text-slate-400 mr-1 shrink-0">Action:</span>
          ...
          <div className="pt-2 border-t border-slate-800/60 flex items-center gap-1.5 overflow-x-auto text-xs font-mono pb-1">
            ...
            {isFiltered && (
              <button
                ...
                className="ml-auto flex items-center gap-1 px-2.5 py-0.5 rounded text-xs font-mono text-amber-400 hover:text-amber-300 hover:bg-slate-800/80 transition-colors shrink-0"
        ...
        {/* Audit Events Table */}
        <div className="border border-slate-800 rounded-xl overflow-hidden bg-[#0a0f1d] shadow-xl">
                ...
                {loading ? (
                  ...
                  filtered.map((ev) => (
                    <tr key={ev.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="py-2.5 px-4 text-slate-400 truncate max-w-[120px]" title={ev.entityId || 'N/A'}>
                        {ev.entityId ? `${ev.entityId.slice(0, 8)}...` : '—'}
                      </td>
                      <td className="py-2.5 px-4 text-slate-400 truncate max-w-[120px]" title={ev.actorId || 'System'}>
                        {ev.actorId ? (
                          <span className="text-slate-300">{`${ev.actorId.slice(0, 8)}...`}</span>
                        ) : (
                          <span className="text-slate-500 italic">system/auth</span>
                        )}
                      </td>
                      ...
                      <td className="py-2.5 px-4 text-right">
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-slate-900 border border-slate-700/80 text-emerald-400 hover:bg-slate-800 text-[11px] transition-colors"
                        >
                          <Code size={12} />
                          <span>View JSON</span>
                        </button>
        ...
        {selectedEventForJson && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
            <div className="w-full max-w-2xl rounded-2xl border border-slate-800 bg-[#0a0f1d] shadow-2xl p-6 space-y-4 text-slate-100">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] font-mono text-slate-500">
                      Action: <span className="text-emerald-400">{selectedEventForJson.action}</span> • ID:{' '}
              ...
              <div className="relative">
                <pre className="p-4 rounded-xl bg-[#070b14] border border-slate-800/90 text-xs font-mono text-emerald-400 overflow-x-auto max-h-96 leading-relaxed select-all">
                  {JSON.stringify(
                    {
                ...
                <button
                  ...
                  className="absolute top-3 right-3 px-2.5 py-1 rounded bg-slate-800 border border-slate-700 text-xs font-mono text-slate-300 hover:text-white flex items-center gap-1"
                >
                  {copiedJson ? (
                    <>
