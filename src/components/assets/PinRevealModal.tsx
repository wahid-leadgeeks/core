...
export default function PinRevealModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-[#0a0f1d] p-6 shadow-2xl space-y-5 text-slate-100">
        {/* Modal Header */}
        ...
        {/* Audit Warning */}
        <div className="p-3.5 rounded-xl bg-amber-950/30 border border-amber-800/60 text-amber-300 text-xs space-y-1">
          ...
          <div className="space-y-4">
            {loginEmail && (
              <div className="space-y-1">
                ...
                <div className="p-2 rounded-lg bg-slate-900 border border-slate-800 text-xs font-mono text-slate-300">
                  {loginEmail}
                </div>
              </div>
            )}
            ...
            <div className="space-y-1">
              ...
              <div className="relative flex items-center">
                <div className="w-full p-4 rounded-xl bg-emerald-950/40 border border-emerald-800/80 text-emerald-400 font-mono text-xl font-bold tracking-widest text-center select-all">
                  {revealedPin}
                </div>
                <button
                  onClick={handleCopy}
                  className="absolute right-3 px-2.5 py-1.5 rounded-md bg-slate-900 border border-slate-700 text-xs font-mono text-slate-300 hover:text-white flex items-center gap-1.5 transition-colors shadow-sm"
                >
                  {copied ? (
                    <>
        ...
        <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-800/80">
          <button
            onClick={onClose}
            className="px-3.5 py-2 rounded-lg bg-slate-900 border border-slate-800 text-xs font-mono text-slate-400 hover:text-slate-200 transition-colors"
          >
            Close
          </button>
          {!revealedPin ? (
            <button
              ...
              className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 disabled:text-slate-600 text-slate-950 font-semibold text-xs font-mono transition-colors shadow-sm flex items-center gap-2"
            >
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-mono transition-colors flex items-center gap-1.5"
            >
              <EyeOff size={13} />
              Mask Secret
            </button>
