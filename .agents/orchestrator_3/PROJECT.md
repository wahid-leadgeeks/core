...
- **Presentation Layer**: Next.js App Router (`src/app/`), calm infrastructure command center UI, Resource Page Pattern, List Page Pattern per `DESIGN.md`. Dark navy sidebar navigation, role switcher, status color badges (🟢🔵⚪🟡🔴).
...
| M1 | App Foundation, Schema & Migrations | Next.js 15, Drizzle ORM, 13 tables, migrations, reference seed scripts | none | DONE (VERIFIED) |
...
| M3 | Spreadsheet Ingestion Engine | 3 XLSX workbooks, normalization, fuzzy matching, PIN encryption, idempotency | M1, M2 | DONE (VERIFIED CLEAN) |
| M4 | Domain CRUD Pages & Navigation UI | Sidebar shell, design system, Identity, Groups, Assets, Software, Audit, Login | M1, M2, M3 | DONE (VERIFIED) |
| Final | 100% E2E Pass & Adversarial Hardening | Phase 1 (100% Tier 1-4 pass) -> Phase 2 (Adversarial coverage hardening) | M4, E2E | IN_PROGRESS |

## Interface Contracts

### M3 ↔ M4: Domain Data for UI Pages
- UI queries Drizzle ORM / API routes:
  - `/accounts` & `/accounts/[id]`: Account list + detail with groups, assets, history tabs. Search & filters (dept, role, type).
  - `/groups` & `/groups/[id]` & `/groups/matrix`: 15 groups, members, 42x15 grid view with active/inactive membership status.
  - `/assets` & `/assets/[id]`: 31 devices, specs, PIC assignment, PIN reveal modal with audit logging. Search & status filters.
  - `/software` & `/software/[id]`: 125 apps, category, department, subscription type (Free/Paid/Freemium) filters.
  - `/audit`: chronological audit events stream with actor, action, entity filters, and JSON metadata viewer. Only Super Admin and Auditor.
