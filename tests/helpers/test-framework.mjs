// Embedded Test Framework and Assertion Library for CORE E2E Test Suite (ESM)
  ...
  toThrow(expectedErrorSubstring) {
    ...
    if (expectedErrorSubstring) {
      ...
      if (expectedErrorSubstring instanceof RegExp) {
        this.assert(
          expectedErrorSubstring.test(msg),
          `Expected error message matching ${expectedErrorSubstring}, got "${msg}"`
        );
      } else {
        this.assert(
          msg.toLowerCase().includes(String(expectedErrorSubstring).toLowerCase()),
...
function parseTierFromName(name) {
