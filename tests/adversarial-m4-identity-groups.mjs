...
import { evaluateRouteGuard, createMockSession, VALID_ROLES } from './helpers/auth-helper.mjs';
...
export function runAdversarialM4Suite() {
  ...
  const resolveGroup = (identifier) => {
    const isUuid = isValidUUID(identifier);
    const decoded = decodeURIComponent(identifier);
    const found = groupsData.groups.find((g) => {
      if (isUuid) {
        return g.id === decoded;
      }
      return (
  ...
  const writeEndpoints = [
    ...
    { path: '/api/accounts/acct-1', method: 'PUT' },
    { path: '/api/accounts/acct-1', method: 'PATCH' },
    { path: '/api/accounts/acct-1', method: 'DELETE' },
    { path: '/api/groups', method: 'POST' },
    { path: '/api/groups/grp-1', method: 'PUT' },
    { path: '/api/groups/grp-1', method: 'DELETE' },
    { path: '/api/assets', method: 'POST' },
    { path: '/api/assets/LGI-CD-2024-001', method: 'PUT' },
    { path: '/api/assets/LGI-CD-2024-001', method: 'DELETE' },
    { path: '/api/assets/LGI-CD-2024-001/credentials/reveal', method: 'POST' },
    { path: '/api/software', method: 'POST' },
    { path: '/api/software/app-1', method: 'PUT' },
    { path: '/api/software/app-1', method: 'DELETE' },
  ];
