...
import { hasPermission, createMockSession, evaluateRouteGuard } from '../helpers/auth-helper.js';
...
describe('Suite 03: Role-Based Access Control (RBAC) & Server-Side Authorization', () => {
  ...
  it('[Tier 1] verifies audit trail deletion is universally forbidden for all roles', () => {
    for (const role of rbacData.roles) {
      expect(hasPermission(role as any, 'audit', 'delete')).toBe(false);
    }
  });
  ...
  it('[Tier 3] pairwise check: Asset Admin vs Software Admin cross-domain mutability', () => {
    ...
    // Asset Admin can create device, cannot create software
    ...
    // Software Admin can create software, cannot create device
  ...
  it('[Tier 4] Scenario 2: Department-scoped administrative delegation', () => {
    ...
    // Asset admin manages hardware inventory
    ...
    // Asset admin cannot access audit or alter Google Groups
  ...
  it('[Tier 4] Scenario 3: Privilege escalation containment', () => {
    ...
    // Software admin attempts to hit credential reveal endpoint directly
