...
const referenceData = JSON.parse(fs.readFileSync(path.join(__dirname, '../fixtures/reference-data.json'), 'utf8'));
...
export function matchPicToAccount(picName, accounts) {
...
export function canonicalizeDepartment(rawName) {
  const map = referenceData.departmentNormalizationMap;
...
describe('Suite 06: Spreadsheet Ingestion Engine, Normalization & Idempotency', () => {
  ...
  it('[Tier 2] secondary custodian (PIC 2) resolution for shared custody laptops', () => {
    const mockAccounts = accountsData.accounts.map(a => ({ displayName: a.displayName, fullName: a.fullName }));
    const sample = devicesData.sampleDevices[2];
