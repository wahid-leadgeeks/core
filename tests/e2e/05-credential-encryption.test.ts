...
import { encryptPin, decryptPin, isEncryptedPin, DEFAULT_TEST_ENCRYPTION_KEY } from '../helpers/crypto-helper.js';
...
describe('Suite 05: Device Credential Encryption (AES-256-GCM) & Secure Reveal', () => {
  ...
  it('[Tier 1] verifies encryption produces distinct IV and ciphertext for identical inputs (semantic security)', () => {
    ...
    // Both decrypt back to same original PIN
  ...
  it('[Tier 2] fails securely when ciphertext is tampered (auth tag authentication failure)', () => {
    ...
    const parts = encrypted.serialized.split(':');
    // Tamper the ciphertext by flipping last hex char
  ...
  it('[Tier 3] pairwise check: Device ingestion + encryption at rest + PIN reveal audit interlock', () => {
    ...
    // 1. Ingestion: encrypt PIN before saving to database
    const encrypted = encryptPin(device.pinPlain);
    expect(isEncryptedPin(encrypted.serialized)).toBe(true);

    // 2. API list response simulation: credentials payload must NEVER return plain text
    const apiDeviceResponse = {
      ...
      credentials: {
        ...
        hasPin: true // scrubbed boolean
      }
    };
    expect((apiDeviceResponse.credentials as any).pinPlain).toBeUndefined();
    expect((apiDeviceResponse.credentials as any).pin_hash).toBeUndefined();

    // 3. Reveal endpoint: Super Admin calls reveal -> receives decrypted PIN
  ...
  it('[Tier 3] pairwise check: Shared PIN vulnerability detection across imported hardware', () => {
    // Spreadsheet data reveals 27 of 31 devices share the identical PIN '123456'
    const identicalPins = devicesData.sampleDevices.filter(d => d.pinPlain === '123456');
    expect(identicalPins.length).toBeGreaterThan(1);

    // Even though inputs are identical, encrypted representations must differ due to unique IVs
  ...
  it('[Tier 4] Scenario 1: IT administrator provisioning a laptop with PIN reveal', () => {
    ...
    const laptop = devicesData.sampleDevices[2]; // Adit's MSI Modern 14

    // 1. IT Admin views device details
    ...
    // 2. IT Admin clicks Reveal PIN button
    ...
    // 3. System decrypts and returns PIN
